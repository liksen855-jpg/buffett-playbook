...
export default async function handler(req, res) {
  ...
  if (req.method === 'PUT' || req.method === 'POST') {
    ...
    try { await kvSet(KEY, { positions, updatedAt: Date.now(), by: session.payload.email || '' }); notifyPositions(positions); res.status(200).json({ ok: true, count: positions.length }); }
