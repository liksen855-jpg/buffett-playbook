...
function scr_getFilters() {
  ...
  return {
    ...
    avgVolMin: gv('scr-avgvol-min'),
    relVolMin: gv('scr-relvol-min'),
...
function scr_needsQuoteEnrich(f) {
  return f.avgVolMin || f.relVolMin || f.epsMin || f.w52 || f.chgMin || f.chgMax || f.peMin || f.peMax;
}
function scr_needsFundEnrich(f) {
  return f.evEbitdaMax || f.pfcfMax || f.fcfYieldMin || f.roeMin || f.roicMin ||
         f.deMax || f.crMin || f.gmMin || f.omMin || f.nmMin ||
...
function scr_updateBadges() {
  ...
  if (f.avgVolMin)    add(`Avg vol ≥ ${Number(f.avgVolMin).toLocaleString()}`, cl('scr-avgvol-min'));
  if (f.relVolMin)    add(`Rel vol ≥ ${f.relVolMin}×`,       cl('scr-relvol-min'));
...
function scr_resetFilters() {
  ...
  const inputs = ['scr-industry','scr-price-min','scr-price-max','scr-vol-min','scr-avgvol-min',
    'scr-relvol-min','scr-beta-min','scr-beta-max','scr-div-min','scr-eps-min',
...
function scr_mergeEnrichment(rows, quoteMap, metricsMap, incomeMap) {
  return rows.map(d => {
    const q  = quoteMap[d.symbol]   || {};
    const km = metricsMap[d.symbol] || [];
    ...
    const avgVol = q.avgVolume || q.averageVolume || 0;
    const relVol = avgVol > 0 ? volume / avgVol : null;
    const eps    = q.eps    || i0.epsdiluted || null;
    const pe     = q.pe     || (price && eps && eps > 0 ? price / eps : null);
    const chgPct = q.changePercentage ?? q.changesPercentage ?? null;
    const yHigh  = q.yearHigh || 0;
    const yLow   = q.yearLow  || 0;
    const w52pos = (yHigh > yLow && price > 0) ? ((price - yLow) / (yHigh - yLow)) * 100 : null;

    const roe       = m0.roe  ?? m0.returnOnEquity           ?? null;
    const roic      = m0.roic ?? m0.returnOnInvestedCapital  ?? null;
    const evEbitda  = m0.evToEBITDA ?? m0.enterpriseValueOverEBITDA ?? null;
    const pfcf      = m0.priceToFreeCashFlowsRatio ?? m0.pfcfRatio ?? null;
    const fcfYield  = m0.freeCashFlowYield ?? null;
    const de        = m0.debtToEquity ?? m0.debtEquityRatio ?? null;
    const cr        = m0.currentRatio ?? null;
    ...
    const eps0 = i0.epsdiluted || 0;
    const eps1 = i1.epsdiluted || 0;
    const epsGrowth = (eps1 !== 0 && eps0 !== 0) ? (eps0 - eps1) / Math.abs(eps1) * 100 : null;

    return {
      ...d, price, volume,
      avgVolume: avgVol, relVolume: relVol,
      eps, pe, changePercentage: chgPct,
      yearHigh: yHigh, yearLow: yLow, w52position: w52pos,
      roe:  roe  !== null ? (Math.abs(roe)  < 2 ? roe  * 100 : roe)  : null,
      roic: roic !== null ? (Math.abs(roic) < 2 ? roic * 100 : roic) : null,
      evEbitda, pfcf,
      fcfYield: fcfYield !== null ? (Math.abs(fcfYield) < 2 ? fcfYield * 100 : fcfYield) : null,
...
function scr_clientFilter(rows, f) {
  ...
  return rows.filter(d => {
    if (n(f.avgVolMin) !== null && (d.avgVolume ?? 0) < n(f.avgVolMin)) return false;
    if (n(f.relVolMin) !== null && (d.relVolume ?? 0) < n(f.relVolMin)) return false;
...
async function runScreener() {
  ...
  let metricsMap = {}, incomeMap = {};
  if (scr_needsFundEnrich(f)) {
    try {
      if (statusEl) statusEl.textContent = `Step 3/3 · fetching fundamentals (${syms.length} tickers)…`;
      [metricsMap, incomeMap] = await Promise.all([scr_batchKeyMetrics(syms), scr_batchIncome(syms)]);
    } catch (e) { console.warn('Fundamentals enrichment failed:', e.message); }
  }

  const enriched = scr_mergeEnrichment(rows, quoteMap, metricsMap, incomeMap);
...
function renderScreenerTable(data) {
  ...
  const hasQuote = data.some(d => d.relVolume !== undefined || d.pe !== null);
  const hasFund  = data.some(d => d.grossMargin !== null || d.roe !== null);
  ...
  const cols = [
    ...
    { key: 'relVolume', label: 'Rel vol', show: hasQuote, th: 'text-align:right',
      td: d => { const rv = d.relVolume;
        return `<td class="${scr_rvClass(rv)}" style="text-align:right">${rv !== null && rv !== undefined ? rv.toFixed(2) + '×' : '—'}</td>`; } },
    ...
    { key: 'pe', label: 'P/E', show: hasQuote, th: 'text-align:right',
